...
const main = async (txHash: string) => {
   ...
   */
  console.log(`L1-to-L2 gas token bridge: ${statusToText[await depositStatus.l1l2GasTokenBridgeRetryable?.status() || 'NA']}`)
  ...
  console.log(`L2-to-L3 token bridge: ${statusToText[await depositStatus.l2l3TokenBridgeRetryable?.status() || ParentToChildMessageStatus.NOT_YET_CREATED]}`)
