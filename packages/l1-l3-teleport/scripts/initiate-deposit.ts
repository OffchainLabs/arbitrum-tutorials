...
const main = async (params: {
  ...
  const gasTokenSymbol = bridger.l2GasTokenAddress ? await ERC20__factory.connect(bridger.l2GasTokenAddress, l2Provider).symbol() : 'ETH'
  ...
  if (depositRequest.gasTokenAmount.gt(0)) {
    ...
    await (await bridger.approveGasToken({
      l1Signer,
      l2Provider,
  ...
  console.log('Done')
  console.log('Initiated deposit! L1-to-L2 retryables will be created, which will create an L2-to-L3 retryable to deposit the tokens.')
...
const args = yargs(hideBin(process.argv))
  .options({
    ...
    skipGasToken: {
      type: 'boolean',
      description: 'Skip paying the L2-to-L3 fee if the L3 doesn\'t use ETH for fees.\nThis has no effect if the L3 uses ETH for fees.',
      default: false
    }
  })
