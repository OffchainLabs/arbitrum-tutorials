...
const main = async () => {
  ...
  switch (args.action) {
    case 'checkConfirmation':
      const confirmations = await checkConfirmation(args.txHash, childChainProvider);
      if (confirmations.eq(0)) {
        console.log('Block has not been submitted to the parent chain yet, please check later...');
      } else {
        console.log(
    ...
    case 'findSubmissionTx':
      ...
      if (submissionTx === '') {
        ...
        console.log(`Submission transaction found on the parent chain: ${submissionTx}`);
      }
      break;
