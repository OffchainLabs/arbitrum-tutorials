...
const main = async (transactionHash) => {
   ...
   */
  const receipt = await childChainProvider.getTransactionReceipt(transactionHash);
   ...
   */
  const messages = await transactionReceipt.getChildToParentMessages(parentChainWallet);
   ...
   */
  if (
    (await childToParentMessage.status(childChainProvider)) == ChildToParentMessageStatus.EXECUTED
  ) {
  ...
  await childToParentMessage.waitUntilReadyToExecute(childChainProvider, timeToWaitMs);
