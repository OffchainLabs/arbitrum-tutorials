...
const main = async () => {
    ...
    const tx = await l2Withdraw.sendTxToL1(l2Wallet.address, "0x", {value: ethFromL2WithdrawAmount })
    const rec = await tx.wait()
    expect(rec.status).to.equal(1)
    
    //Option 2-- withdrawEth(_destAddress):
    const tx = await l2Withdraw.withdrawEth(l2Wallet.address,  {value: ethFromL2WithdrawAmount })
    const rec = await tx.wait()
    expect(rec.status).to.equal(1)
